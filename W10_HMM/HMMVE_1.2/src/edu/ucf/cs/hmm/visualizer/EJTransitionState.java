...
/**
 * Lines from E state to J state
 *
 */
class EJTransitionState extends TransitionState {
	Point start;
	...
	void generateShape(int _size) {
		size = _size;
		line1Shape = new Line2D.Float(start.x, start.y, start.x + size, start.y
				+ size);
		line2Shape = new Line2D.Float(start.x + size, start.y + size, start.x
				+ size, end.y);
		line3Shape = new Line2D.Float(start.x + size, end.y, end.x, end.y);
		arrowShape = generateArrow(start.x + size, end.y, end.x, end.y);
	}
	...
	public void drawText(Graphics2D g2d, Rectangle2D clip, int precision) {
		if (show) {
			...
			g2d.drawString(String.format("%." + precision + "f", p), start.x
					- size * 2, end.y - size / 2);
		}
	}
	/**
	...
	protected Object clone() {
		EJTransitionState result = new EJTransitionState();
		result.p = p;
