...
/**
 * Lines from M state to E state
 *
 */
public class EndTransitionState extends TransitionState {
	Point start;
	...
	void generateShape(int _size) {
		size = _size;
		line1Shape = new Line2D.Float(start.x, start.y, start.x + size, end.y);
		line2Shape = new Line2D.Float(start.x + size, end.y, end.x, end.y);
	...
	protected Object clone() {
		EndTransitionState result = new EndTransitionState();
		result.p = p;
